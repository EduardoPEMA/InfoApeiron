	...
	public void run() {
		...
		while(true){
			...
			try {
				...
				promedio=0;
				for (int x=0;x<velocidades.length;x++)
					promedio+=velocidades[x];
...
class URLParts{
	...
	public static URLParts parseURL(String url){
		...
		if (url.startsWith("http://")){
			...
			if (host.equals(""))
				throw new HttpReaderException("URL Inválida: " +url);
...
public class HttpReader {
	...
	public synchronized HttpResponseHeader getHeader(String url)throws IOException{
		...
		if (direccion==null)
			direccion=skt.getLocalSocketAddress();
	...
	public synchronized String get(String url)throws IOException{
		...
		try{
			do{
				...
				res=is.read(buffer,pos,bsize);/*buffer.length-pos);*/

	...
	public Preferencias(AdDatos d,Principal prin){
		...
		tmGrupos=new AbstractTableModel(){
			...
			public Object getValueAt(int rowIndex, int columnIndex) {
				if (maestro!=null){
					...
					switch (columnIndex){
						case 0: return grp.getNrc();
						case 1: return grp.getSec();
						case 2: return Integer.toString(grp.getCup());
						case 3: return Integer.toString(grp.getDis());
						case 4: return grp.getHor();
						case 5: return grp.horario.getPeriodoString();
					}
					return "";
				}else
					return "";
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ret==JFileChooser.APPROVE_OPTION){
					...
					try{
						...
						JOptionPane.showInternalMessageDialog(principal.preferencias,e2+"","Error",JOptionPane.ERROR_MESSAGE);
					}
				}
		...
		btnExportar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				if (ret==JFileChooser.APPROVE_OPTION){
					if (fc.getSelectedFile().getName().endsWith(".xml") || fc.getSelectedFile().getName().endsWith(".xml"))

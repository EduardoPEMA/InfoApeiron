...
public class LectorSiiau extends LectorHtml implements LectorHtmlListener{
	...
	private void procesar4(HtmlTag horario,Registro r){
		...
		//String ses=horario.getSubTag(0).contenido;
	...
	public void terminado(HtmlTag arbol){
		...
		//Buscar en donde empieza la tabla de informacion
		HtmlTag tabla=arbol.buscar("nrc",2);
		HtmlTag reg; 
		if (tabla!=null){
			//Empieza la seleccion de campos
			...
			rutaHor=reg.buscarRuta("Ses/Hora/D&iacute;as/Edif/Aula/Periodo",0);
			rutaProf=reg.buscarRuta("Ses/Profesor",0);
			...
			if (rutaNrc==null ||
				...
				throw new RuntimeException("El formato ha sido actualizado por favor contactanos");
			invReg=0;
			procesar(tabla);
			if (invReg>0)
				for(int x=0;x<eventos.size();x++){
					((LectorSiiauListener)eventos.get(x)).advertencia("Registros inválidos: "+Integer.toString(invReg));
				}
				
			
		}else
			throw new RuntimeException("No se encontro la tabla de información");

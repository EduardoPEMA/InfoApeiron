...
public class HtmlParser {
	...
	private void leerRecursivo(String tag,HtmlTag info){
		while(indice<tokens.size()){
			if (eventos!=null)
				eventos.progreso("Análisis sintáctico...",indice*100/tokens.size());
			...
			if (!actual.tag.equals("")){
				if (actual.tagCierra){
					if (actual.tag.equals(tag)){
						...
						indice--;
						//System.out.println("No se encontro final: "+tag);
						return;
					}
				}else{//Aqui abre
	...
	public HtmlTag parse(String s){
		...
		for (indice=0;indice<s.length();indice++){
			...
			if (eventos!=null)
				eventos.progreso("Análisis léxico",(indice+1)*100/s.length());
		}
		...
		leerRecursivo("fin",n);
		if (eventos!=null)
			eventos.progreso("Análisis Global",90);
		filtrarEtiquetas(n);
		filtrarContenido(n);
		if (eventos!=null)
			eventos.progreso("Análisis Global",100);

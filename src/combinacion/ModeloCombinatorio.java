...
public abstract class ModeloCombinatorio extends Thread {
	...
	/*private void quickSort(int primero, int ultimo,Object soluciones[]){
		
		if (primero>=ultimo)
			return;
		
		int pivote=((Solucion)soluciones[(primero+ultimo)/2]).horario.getHuecos();
		int i=primero;
		int j=ultimo;
		
		do{
			while (((Solucion)soluciones[i]).horario.getHuecos()<=pivote && i<ultimo)
				i++;
			while (((Solucion)soluciones[j]).horario.getHuecos()>=pivote && j>primero)
				j--;
			
			if (i<=j){
				Object tmp=soluciones[i];
		...
		if (primero<j)
			quickSort(primero,j,soluciones);
		
		if (i<ultimo)
			quickSort(i,ultimo,soluciones);
	...
	private void ordenaPorDemanda(boolean inverso){
		...
		if (!inverso){
			for(int z=0;z<materias.length;z++){
				for (int x=materias[z].length-2;x>=0;x--)
					for (int y=x; y<=materias[z].length-2;y++){
	...
	private void ordenaPorNdGrupos(boolean inverso){
		if (inverso){
			for (int x=materias.length-2;x>=0;x--)
				for (int y=x; y<=materias.length-2;y++){
			...
			for (int y=0;y<mat.maestros.size();y++){
				...
				if (maestro.getMarca()){ //Filtrar por maestro
					for (int z=0;z<maestro.grupos.size();z++){
						...
						if ((grp.getDis()>0 || conCupo==false) && grp.subHorario(horarioUsuario)) 
							grps++;
					}
				}
			}
			...
			for (int y=0;y<mat.maestros.size();y++){
				...
				if (maestro.getMarca()){ //Filtrar por maestro
					for (int z=0;z<maestro.grupos.size();z++){
						...
						//Filtrar por cupo y por horario del usuario
						if ((grp.getDis()>0 || conCupo==false) && grp.subHorario(horarioUsuario)) 
							materias[x][idx++]=(Grupo)maestro.grupos.get(z);
					}
				}
			}
		}
	...
	public void run() 
	{
		try
		{
			...
			if (solCount>0){
				...
				for (int x=0;x<90;x++){
					if (hashHuecos[x]!=null)
						for (int i=0;i<hashHuecos[x].size();i++)
							arrSol[indice++]=hashHuecos[x].get(i);
				}
				fireProgreso("Proceso terminado al",-3);
			}else
				fireProgreso("No se encontro ningun horario. Trata cambiando las Preferencias",-3);
		}
		catch (OutOfMemoryError e)
		{
			...
			solCount=0;
			System.gc();
			fireProgreso("Falta Memoria [Cierra ventanas y limita el número de horarios a generar]",-3);
		}
	}
